...
$$Cost =  \sum^n_{i=1}(y_i-x_i^T\vec{\beta})^2+\lambda\sum^p_{j=0}|\beta_j| $$ 
Where $x_i^T$ is the covariates for observation i. $\lambda$ is a hyperparameter controlling the strength of the lasso regularization. $\vec{\beta}$ is a 1xp vector with the regression coefficients $$\vec{\beta} = \begin{bmatrix}
\beta_0 \\
\beta_1 \\
\vdots \\
\beta_p
...
# This extracts the vector of complexity‑parameter values from the complexity‑parameter 
# table (cptable) stored in the rpart fit object.
# We're iterating through pruned trees stored in this table which should allow us to
# determine the optimal number of leaves
