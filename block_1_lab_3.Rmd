...
distance_to_station <- sapply(1:nrow(st), 
                              function(x) distHaversine(c(st$longitude[x], 
                                                          st$latitude[x]), 
...
prior_df$delta_t <- sapply(1:nrow(prior_df), 
                           function(x) hour_fun(numeric_hour_of_interest,
                                                prior_df$time[x] |> substr(1, 2) |> as.numeric()))
...
a <- sum(dnorm(prior_df$distance / h_distance) * prior_df$air_temperature) +
  sum(dnorm(prior_df$day_diff / h_date) * prior_df$air_temperature) +
  sum(dnorm(prior_df$delta_t / h_time) * prior_df$air_temperature) 

b <-  sum(dnorm(prior_df$distance / h_distance)) +
  sum(dnorm(prior_df$day_diff / h_date)) +
  sum(dnorm(prior_df$delta_t / h_time))
...
c <- dnorm(prior_df$distance / h_distance) * dnorm(prior_df$day_diff / h_date) *
  dnorm(prior_df$delta_t / h_time) 
...
- filter0
  - Trained on tr and Evaluated on va
